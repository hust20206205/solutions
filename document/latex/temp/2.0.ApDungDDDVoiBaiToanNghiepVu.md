...
Trong microservice  Các dịch vụ   cần tương tác với các dịch vụ  khác.
=>  Sẽ có những mối quan hệ phụ thuộc.   Những mối quan hệ này cần được quản lý. 
Nếu không
mất đi những lợi ích mà bạn mong đợi nhận được từ kiến ​​trúc dịch vụ vi mô.
 thì sẽ mất tính toàn vẹn và sẽ mất đi khả năng hoạt động độc lập của nhóm.
...
=> Vì vậy, đề xuất là quản lý các liên hệ, mối quan hệ được liên kết này bằng cách sử dụng các mẫu thiết kế hướng miền thích hợp. Các đội phải nỗ lực phải ghi lại mối quan hệ giữa các điểm tiếp xúc được liên kết bằng cách sử dụng Bản đồ bối cảnh (Context Maps)
